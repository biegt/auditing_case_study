...
Der zurückgegebene Dataframe enthält 0 Zeilen. Also lässt isch annehmen, dass die Differenz von Anfangs- und Endsaldo zwischen dem Buchungsjournal und der Saldenliste für jedes Konto übereinstimmt.
...
Im folgenden werden weiterführende Analysen mit Schwerpunkt Betrugsermittelung durchgeführt. Ziel ist es Indizien zu für dolose Handlungen / betrügerische Aktivitäten zu finden. 

### Prüfungen der buchenden Personen
Insgesamt haben `r length(unique(je_2014$preparer_id))` Benutzer durchgeführt.
...
je_2014 %>% count(weekday_efficient_date) %>%
  mutate(
    weekday_efficient_date = fct_relevel(
      weekday_efficient_date,
      weekday_vec
    )
  ) %>%
...
je_2014 %>% count(weekday_entry_date) %>%
  mutate(
    weekday_entry_date = fct_relevel(
      weekday_entry_date,
      weekday_vec
    )
  ) %>%
...
je_2014 %>%
  ...
  scale_y_log10(name = "Funktionaler Betrag (absolut)",
...
# je_2014_gt_10 <- je_2014[je_2014["functional_amount"] >= 10.0,]
```

Benford-Analyse:

"Naive" Benford-Analyse (Two-Digit-Test) für das gesamte Dataset:

```{r]}
bf_total <- benford(je_2014$functional_amount)
bf_total
plot(bf_total)

# getDuplicates(benford_test, je_2014_positive) %>%  View()
# 
# benford_3 <- benford(je_2014_test_positive$functional_amount, number.of.digits = 3)
# 
# benford(je_2014_test_gt_10$functional_amount, number.of.digits = 3)
# 
# extract.digits(je_2014_test_positive$functional_amount, number.of.digits = 3, digit)
```
...
users <- je_2014 %>% count(preparer_id, sort = TRUE) %>% pull(preparer_id)

map(users, ~ bf_by_user(.)$MAD.conformity)
...
## Nächste Schritte: 
* bf_by_user so implementieren, dass auch ursprünglichen Datenabrufbar sind
